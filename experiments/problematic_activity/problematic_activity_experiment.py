from code.algorithms.heuristics_hillclimber import ProblematicActivityClimber
...
def problematic_activity_all_averages(schedule, nr_climbers: int =30, nr_iterations: int =20000):
    ...
    for i in range(nr_climbers):
        ...
        climber = ProblematicActivityClimber(schedule)
    ...
    with open(f"results/problematic_activity/problematic_activity_all_averages-{nr_climbers}-{nr_iterations}.csv", 'w', newline='') as output_file:
...
def problematic_activity_ratios_plot(nr_climbers: int =30, nr_iterations: int =20000):
    ...
    df = pd.read_csv(f'results/problematic_activity/problematic_activity_all_averages-{nr_climbers}-{nr_iterations}.csv', names=names)
    ...
    plt.title(f'Maluspoints of n={nr_climbers} Problematic Activity Hillclimbers')

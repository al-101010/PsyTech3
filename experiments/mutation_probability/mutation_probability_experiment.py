from code.algorithms.heuristics_hillclimber import MutationProbabilityClimber
...
def mutation_probability_all_averages(schedule, nr_climbers: int =30, nr_iterations: int =20000):
    ...
    for i in range(nr_climbers):
        ...
        climber = MutationProbabilityClimber(schedule)
        ...
        get_output(climber.schedule.students, dir_path+f'/mutation_probability{i + 1}_output.csv')
    ...
    with open(f"results/mutation_probability/mutation_probability_all_averages-{nr_climbers}-{nr_iterations}.csv", 'w', newline='') as output_file:
...
def mutation_probability_ratios_plot(nr_climbers: int =30, nr_iterations: int =20000):
    ...
    df = pd.read_csv(f'results/mutation_probability/mutation_probability_all_averages-{nr_climbers}-{nr_iterations}.csv', names=names)
    ...
    plt.title(f'Maluspoints of n={nr_climbers} Mutation Probability Hillclimbers')
